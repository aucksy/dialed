...
| **2D-vis** | `dialed-v0.25.0` → **`v0.26.0`** ✅ | **Collections Home (visual browse spine)** — collection cards + `CollectionScreen`, derived from `Face.series`; no paywall/config/entitlement. v0.26.0 = **fidelity fix** (big vitrine hero cards, owner-signed-off Variant A) + F5 press + F2-flavoured expand motion | 2C |
| **2D** | *next free* | **Collections IA commercial layer** — `config/catalog.json` + free faces + coming-soon tiles + entitlement-v2 (app). ⚠ **BLOCKED** — needs the 2A map signed off (audit §11 q1/q3/q4). | 2A, 2D-vis |
...
| **— Guided first-face hand-off** | v0.24.0 | ✅ **SHIPPED** 2026-07-18 | **Out-of-band, `:wear` UI only** (bridge/repo/listener/ViewModel/`:wear-common` byte-identical). Owner: first face never auto-applies → make it better or give a set-button. Researched to ground truth (Google WFP docs + API surface + androidify): reliable first auto-apply is a **hard platform wall** (once-ever unattended `setWatchFaceAsActive`; no picker Intent; no repeatable variant; androidify has the same limit + manual fallback). Owner chose "smooth the hand-step now". Built `GuidedHandoff` (spec 1m/W3): the refused-activation screen now shows the face (50dp) + 3 personalised gestures + gentle 1→2→3 highlight (reduced-motion static) on `ScreenScaffold`+`verticalScroll`, and routes straight to the watch face — never faking a bypass. **Real long-term fix = bundled default face (Phase 5), deferred by owner.** |
...
| **2D-vis Collections Home (visual)** | v0.25.0 → **v0.26.0** | ✅ **SHIPPED** 2026-07-18 | The **IA/visual slice** of 2D, split off so it needs no parked decision. Home is a scroll of **10 collection cards**, grouped by `Face.series`; tap → `CollectionScreen` (the existing 150dp showroom grid scoped to the collection + a "one face at a time" footer); Back returns to the collection then Home (`Screen.Collection` + `Detail.fromCollectionId` + Saver + M1-class fallbacks). **No paywall, prices, lock badges, config, coming-soon or entitlement-v2** — collections are **derived** (`catalog/Collection.kt`), browse surfaces are entitlement-agnostic, the detail install gate is untouched (release behaviour == v0.24.0). `:wear` diff = lockstep version bump only. ⚠ **v0.25.0's cards were too small** (60dp previews built from prose, not the spec CSS — owner rejected). **v0.26.0 = fidelity fix**: owner-signed-off (true-scale mock) **Variant A "vitrine hero cards"** — large overlapping trio (centre 132dp/flanks 96dp) over name/style/count; **F5 press + F2-flavoured Material-Z expand** (true shared-element morph deferred to 2E, needs on-device verify). ⭐**Lesson: read `.dc.html` in full, never `grep`/`sed` (strips the CSS/sizes); mock → owner sign-off → code.** Spec: `docs/DESIGN-ADDENDUM-COLLECTIONS.md` §3/§5. |
| **2D Collections IA (commercial)** | *next free* | 🚧 **BLOCKED** on the 2A map | The rest of 2D on top of the shipped v0.25.0 spine: which faces are FREE; `config/catalog.json` + `CatalogConfigRepository` (replaces the derivation); coming-soon tiles + Chrome-rendered art; entitlement-v2 (per-collection set); per-collection unlock CTA on detail. Keep the `:wear` diff at **zero**. ⚠ **Cannot start**: the shelves' commercial shape is the collection map, whose questions (audit §11 **q1** Vakt 15-vs-10, **q3** part-built pricing, **q4** card total-vs-split) were parked by the owner 2026-07-17. Pricing is *not* the blocker (the paywall is a stub until Phase 3) — the **map** is. |
