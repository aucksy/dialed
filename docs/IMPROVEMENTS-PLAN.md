# Dialed — Quality & Features Plan (v0.3.0 → v0.10.0) — ⚠️ SUPERSEDED

> **⚠️ HISTORY ONLY — this roadmap is complete and closed.** Every issue it tracked has shipped
> (see the Progress table at the bottom, which now runs through `dialed-v0.19.0`). It is kept for the
> root-cause write-ups and the per-tag record.
>
> **The ACTIVE roadmap is `docs/ASSESSMENT.md` + `docs/IMPLEMENTATION-PLAN.md`** (2026-07-15
> end-to-end assessment of both apps → 8 phases: hygiene ✅ → collections IA → billing → store →
> default face → living gallery → Collection 3 → wear polish). Start there.
>
> The one item this doc left open — **Phase 6 carousel/favorites** — was NOT abandoned: R4 proved it
> is a platform limit whose only lever is a bundled default watch face, and that work is now the new
> plan's **Phase 5**.

**Original framing —** Owner reported 8 major issues after on-device testing `dialed-v0.2.1`
(Pixel Watch 4 42mm + phone). This planned them in phases. Entry point: `CLAUDE.md` (architecture,
WFP gotchas, ship loop). Design source of truth: the two `*.dc.html` specs + `HANDOFF*.md`.
...
| Regressions batch (owner re-test) | v0.10.0 | 🚧 **SHIPPING** 2026-07-12 (`dialed-v0.10.0`) | Owner on-device found 4 issues after v0.9.0. **(A) Phone "not compatible / can't install" (REGRESSION):** CI debug-signed `:app` with an EPHEMERAL per-run keystore → every release had a different signature → in-place upgrade rejected (proved: v0.8.0 cert `385d6df9` vs v0.9.0 `70520e2a`). **Fix = committed STABLE key `dialed-app-debug.keystore`** signing both `:app`+`:wear` (distinct from `dialed-faces.keystore`, WFP rule ok; shared so phone/wear pair). Owner must clean-install v0.10.0 ONCE (adopts the stable key); upgrades work in place thereafter. **(#4) Wear falsely "Connected":** used `NodeClient.connectedNodes` (any paired phone). Now phone advertises `dialed_phone_app` capability (app `res/values/wear.xml`) + wear checks `CapabilityClient.getCapability(CAPABILITY_PHONE, FILTER_REACHABLE)` + live listener → CONNECTED only when the Dialed phone app is actually reachable. **(#2) First install not auto-applied:** `SET_PUSHED_WATCH_FACE_AS_ACTIVE` is a RUNTIME permission (grant-once, no true no-user-consent) → now requested UPFRONT in onboarding (right after PUSH) so the first push computes `CALL_SET_ACTIVE_NO_USER_ACTION` → auto-applies + branded Celebration covers the platform apply. **(#3) Wear preview oval:** previews are 450² square + Dialed clips circle → the oval is the PLATFORM system apply-thumbnail (R5), not Dialed; defensive `centerSquare()` in `FacePreviewExtractor` guarantees Dialed surfaces stay circular, and #2's auto-apply Celebration now covers the platform moment on first install. **(#1) Fake gold seconds hand REMOVED** from the detail hero (`ticking=false`); the REAL per-face second hand is v0.11.0 (only 7/18 faces are analog). |
...
| 6 Carousel/favorites | — | ➡️ **MOVED** | R4 DEFINITIVE: **no favorites/carousel API**; slot=1; the only lever is bundling a **default watch face** → seeds ONE tile in the system gallery. Never shipped under this roadmap; it is now **Phase 5 of `docs/IMPLEMENTATION-PLAN.md`** (R4's RECIPE section is execution-ready). |

### After this roadmap closed — the v0.11 → v0.19 record

| Ver | Date | What shipped |
|---|---|---|
| `dialed-v0.11.0` / `v0.11.1` | 2026-07-12/13 | **Complication icon-label REFERENCE (Aether-Ember).** Each hardcoded `<Default>` label replaced by the provider's own icon (`[COMPLICATION.MONOCHROMATIC_IMAGE]`, tinted, AMBIENT alpha-0) so the label swaps with the provider — fixes the "still says Steps" bug. v0.11.1 dropped `isAutoSize` (a **WFF v3+** attr — these faces are v2, so it fails validation; recurring trap). Owner-confirmed on-wrist. |
| `dialed-v0.12.0` | 2026-07-13 | **Icon-label ROLLOUT to the other 15 flagship faces** (56 label sites; Arclight pair was already on the recipe). Scripted per-face XML transform scoped strictly inside `<Complication>` blocks (decorative face text never touched); handles both label shapes (Condition+`[COMPLICATION.TITLE]`, and Vespera-Aurum's bare `<PartText>`); layout-aware STACKED vs SIDE placement. |
| `dialed-v0.13.0` | 2026-07-13 | **Wear landing-screen fixes.** Preview oval → `FaceDial` now uses `requiredSize` + an explicit center-cropped Canvas `drawImage` (a `CircleShape` clip on a parent-coerced non-square box renders an **ellipse**). "Connected" flapping → retry probes + a 12s periodic self-heal (`FILTER_REACHABLE` is a transient snapshot that races the Data Layer's initial sync). |
| `dialed-v0.14.0` | 2026-07-15 | **Wear HomeScreen rebuilt from the design spec.** One root cause, three symptoms: a vertically-centred `Column` overflows a 192dp round screen → status clipped off the top, name off the bottom, face squeezed high. Fix = `DialedHeroScreen` (full-height `BoxWithConstraints`, EdgeButton overlays the bezel) + fractional anchors per spec §A (.14 / .245 @ 73dp / .656). **Lesson: the design-fidelity gate applies to the wear UI too.** |
| `dialed-v0.15.0` | 2026-07-15 | **Auto-apply reliability + exit-to-watch-face.** Verified against 3 primary sources: the unattended `setWatchFaceAsActive` is a **platform once-ever anti-hijack budget**; auto-apply then survives only via the ownership chain (updating the slot you already own) — which is exactly why "the ones right after" apply but the first-of-day doesn't. Removed the pessimistic local latch (a local latch can only be *more* pessimistic than the platform); a refused set now maps to the honest manual coach, never a false "Dialed in."; Celebration exits to the freshly-applied face. |
| `dialed-v0.16.0` | 2026-07-15 | 🚨 **Regression fix: WFP calls could wedge the install → leak the transfer lock → "your watch is busy" forever.** Latent since the transport was built; a cold WFP service on a fresh install exposed it. Fix = `withTimeoutOrNull` on EVERY WFP binder call (install 30s / setActive 12s / queries 8s, all << the phone's 120s finalize wait) + rethrow `CancellationException` before the broad catch (which had been swallowing timeout cancellation and defeating the timeouts). |
| `dialed-v0.17.0` | 2026-07-15 | **Home conveys the GENUINE WFP slot state** (was: last-pushed cache + a hardcoded "ACTIVE" label, so it claimed faces that weren't even installed). Three real states from `installedState()`: none / installed+active / installed+inactive (+ a local "Set as your face" action). Anti-stale rule: friendly name + cached preview only when the stored package matches what WFP reports installed. |
| `dialed-v0.18.0` | 2026-07-15 | **"Open/Browse on phone" actually opens the phone app** (it was cosmetic — `OpenOnPhoneDialog` is a "check your phone" *animation* with no action). Fix = `RemoteActivityHelper.startRemoteActivity` (the only BAL-safe path) with a `dialed://open` VIEW/BROWSABLE deep link + a matching intent-filter on the phone. |
| `dialed-v0.19.0` | 2026-07-15 | **Hygiene batch — Phase 1 of the NEW plan** (`docs/IMPLEMENTATION-PLAN.md`), from the end-to-end assessment: **C1** the release paywall could grant a free permanent unlock (now `BuildConfig.DEBUG`-gated); **H2** concurrent-push race + cancellation hygiene; **H3** uninstall failures surfaced; **H4** honest unsupported-watch path (`RESPONSE_UNSUPPORTED` byte + query sentinel → the UNSUPPORTED pill finally has a producer); **M1** restored-detail crash guard; **M3** phone `singleTask`; **M4** dead controls + real version string; **M5** 48dp targets + pill live region; **M6/L1** dead code; **H6** this docs refresh. |
...
**This roadmap is closed — do not add to it.** Track new work in `docs/IMPLEMENTATION-PLAN.md`'s
Progress table instead. The working rhythm is unchanged: one phase per fresh chat, finish → adversarial
review → tag → paste the APK links → hand off the next kickoff prompt.
