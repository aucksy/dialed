...
## Phase 2 — Face resolution + preview distortion  _(faces submodule + Dialed; depends on R1, R5)_ → **`dialed-v0.9.0`** (screenshot-gated; floated past Phases 3, 4, 4b, 5)
...
| 2 Resolution + distortion | v0.9.0 | ⬜ | **Screenshot-gated → floats (now past Phase 6 too).** #5 distortion = platform UI, NOT an app bug (closed). #1 resolution still needs a worn-face screenshot ± the R1 300→450 twin test. The only remaining faces-side item; unblocks the moment the owner sends a photo of the worn, applied face. |
