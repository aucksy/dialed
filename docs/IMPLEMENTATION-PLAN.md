...
| **2D-vis** | `dialed-v0.25.0` ✅ | **Collections Home (visual browse spine)** — collection cards + `CollectionScreen`, derived from `Face.series`; no paywall/config/entitlement | 2C |
...
| **2D-vis Collections Home (visual)** | v0.25.0 | ✅ **SHIPPED** 2026-07-18 | The **IA/visual slice** of 2D, split off so it needs no parked decision. Home is now a scroll of **10 collection cards** (cover trio + name + subtitle + "N faces"), grouped by `Face.series`; tap → `CollectionScreen` (the existing showroom grid scoped to the collection + a "one face at a time" footer); Back returns to the collection then Home (`Screen.Collection` + `Detail.fromCollectionId` + Saver + M1-class fallbacks). **No paywall, prices, lock badges, config, coming-soon or entitlement-v2** — collections are **derived** (`catalog/Collection.kt`), browse surfaces are entitlement-agnostic, the detail install gate is untouched (release behaviour == v0.24.0). `:wear` diff = lockstep version bump only. Spec: `docs/DESIGN-ADDENDUM-COLLECTIONS.md`. |
