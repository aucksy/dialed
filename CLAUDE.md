...
- ⏭️ **Next: NOT 2D — it is BLOCKED.** Owner steer 2026-07-17: pricing is parked to last ("this will be last state"); the priority is **testing and perfecting the existing faces**. 2D can't start because its shelves *are* the 2A collection map, whose shape questions (audit §11 q1 Vakt 15-vs-10, q4 card total-vs-split) are parked. Pricing is not the blocker (the paywall is a stub until Phase 3) — the **map** is. **Live candidate = 2B colour parity** (16 faces at 3 ColorOptions → 5), which fits the steer and needs no parked decision except Pulsar's (§11-q2).
