...
- ✅ **v0.28.0 = FIRST-APPLY FIX: bundled default face + ownership chain** (`docs/FIRST-APPLY-SOLUTION.md`
  executed end-to-end). New **`:watchface`** module — the repo's FIRST hand-authored (non-fablecollection)
  WFF v1 face @480: a clean Dialed-branded digital dial (gold ring, champagne `h:mm`, baked "DIALED"
  wordmark, gold accent; ambient = dim time via the interactive-α0/aod-α255 twin), pkg
  `com.dialed.app.watchfacepush.dialed.classic`, gallery name "Dialed", faces-key signed. CI builds +
  validates it (first run PASSED) and stages `apk`+`token` into `wear/src/main/assets/default_watchface.*`
  (gitignored, CI-fresh) BEFORE the `:wear` build. Onboarding **`MakeDefaultScreen`** installs it + spends
  the one-shot `setActive` (`WearViewModel.makeDefaultFaceActive`) so Dialed owns the active slot → every
  later push auto-applies. Uninstall **reverts the slot to the default via `updateWatchFace`** instead of
  `removeWatchFace` (`WatchFacePushRepository.removeByPackage`; real-remove fallback) — the ONE deliberate
  transport-adjacent change; listener/channel/`TransferSession`/`:wear-common` byte-identical. Concierge
  redesigned: `OneTapApply` 80dp→108dp big-face hero, `GuidedHandoff` 50dp→84dp. ⚠ **WFF authoring traps
  (reuse):** `<Template>` needs a numeric `<Parameter>` so a static wordmark must be a BAKED bitmap;
  `resource=` in `res/raw/watchface.xml` must be a BARE name (validator bug) but `@drawable/` is correct in
...
- `DEFAULT_WATCHFACE_VALIDATION_TOKEN` marketplace-auto-install meta-data is intentionally NOT used (v0.28.0): the bundled default (`:watchface`) is installed + activated explicitly at the "Make Dialed your watch face" onboarding moment, not blindly at app-install. See the `wear/src/main/AndroidManifest.xml` comment.
